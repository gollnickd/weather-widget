...
async function loadDashboardStats() {
  try {
    ...
    document.getElementById('total-locations').textContent = 'Error';
    document.getElementById('api-calls-today').textContent = 'Error';
  }
}
  ...
  try {
    ...
    tbody.innerHTML = customers.map(customer => `
      <tr>
        ...
        <td>${customer.website_url ? `<a href="${customer.website_url}" target="_blank">${customer.website_url}</a>` : 'N/A'}</td>
        <td>${customer.contact_email || 'N/A'}</td>
        <td><code style="font-size: 11px;">${customer.api_key}</code></td>
        <td>${customer.location_count || 0} locations</td>
        <td>
    ...
    const tbody = document.querySelector('#customers-table tbody');
    tbody.innerHTML = '<tr><td colspan="7" style="text-align: center; color: #EF4444;">Error loading customers: ' + error.message + '</td></tr>';
  }
}
